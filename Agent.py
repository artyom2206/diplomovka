...
class Agent:
    def __init__(self, input_shape, action_space, game_name,
        ...
        if not self.ddqn and not self.dueling:
            self.game = self.game + "_DQN"
        elif self.ddqn and not self.dueling:
            self.game = self.game + "_DDQN"
        elif not self.ddqn and self.dueling:
            self.game = self.game + "_Dueling"
        elif self.ddqn and self.dueling:
    ...
    def save(self):
        self.policy_network.save(filepath=str("Weights/" + self.game + "_weights"))
